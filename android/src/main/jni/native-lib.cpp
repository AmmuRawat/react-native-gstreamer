...
//     cv::Mat& prevGray, cv::Mat currFrame, cv::Mat currGray, gpointer user_data, 
//     int width, int height)
//      {
//         CustomData *data = static_cast<CustomData*>(user_data); // Cast user_data to CustomData*
//         cv::Mat f_stabilized = frame.clone(); // Default return value
//         try {
//         // Input validation
//         if (!ret || frame.empty() || currFrame.empty() || currGray.empty()) {
//             throw std::runtime_error("Invalid input frame(s)");
//         }

//         // Resolution calculations
//         double res_w_orig = static_cast<double>(width);
//         double res_h_orig = static_cast<double>(height);
//         double res_w = res_w_orig * downSample;
//         double res_h = res_h_orig * downSample;

//         // Zoom transformation matrix
//         cv::Mat T = cv::getRotationMatrix2D(cv::Point2f(res_w_orig / 2, res_h_orig / 2), 0, zoomFactor);

//         // Initialize previous frames if empty
//         if (prevFrame.empty()) {
//             prevOrig = frame.clone();
//             prevFrame = frame.clone();
//             prevGray = currGray.clone();
//             lastRigidTransform = cv::Mat::eye(2, 3, CV_64F); // Identity transform
//             return f_stabilized;
//         }
...
//         if (prevPts.empty()) {
//             throw std::logic_error("No good features to track in previous frame");
//         }

//         std::vector<uchar> status;
//         std::vector<float> err;
//         cv::calcOpticalFlowPyrLK(prevGray, currGray, prevPts, currPts, status, err, winSize, 3, termcrit, 0, 0.001);

//         if (currPts.empty()) {
//             throw std::logic_error("No corresponding points in current frame");
//         }
...
//         if (prevPts_rescaled.size() < 3 || currPts_rescaled.size() < 3) {
//             throw std::logic_error("Insufficient tracked points");
//         }

//         // Estimate transformation
//         cv::Mat m = cv::estimateAffinePartial2D(prevPts_rescaled, currPts_rescaled);
//         if (m.empty()) {
//             m = lastRigidTransform.clone();
//         } else {
...
//         // Stabilization using Kalman filter
//         cv::Mat Z = (cv::Mat_<double>(1, 3) << x, y, a);
//         if (count1 == 0) {
//             X_estimate = cv::Mat::zeros(1, 3, CV_64F);
//             P_estimate = cv::Mat::ones(1, 3, CV_64F);
...
//         cv::Mat_<double> m_new = (cv::Mat_<double>(2, 3) << cos(da), -sin(da), dx,
//                                                           sin(da),  cos(da), dy);
//         cv::warpAffine(prevOrig, fS, m_new, cv::Size(res_w_orig, res_h_orig));
//         cv::warpAffine(fS, f_stabilized, T, cv::Size(fS.cols, fS.rows));
//     } catch (const std::exception& e) {
//         std::cerr << "Error: " << e.what() << std::endl;
//         f_stabilized = frame.clone(); // Fall back to original frame
//     }

//     // Synchronization using mutex and condition variable
//     {
