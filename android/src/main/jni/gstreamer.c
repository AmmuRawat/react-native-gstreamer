...
typedef struct _CustomData {
 ...
 jobject surface_object; /* Java surface object */
} CustomData;
 ...
 * Private methods
 */
static void gst_native_restart_pipeline(CustomData *data);
...
error_cb (GstBus * bus, GstMessage * msg, CustomData * data)
{
 ...
 gst_native_restart_pipeline(data);
}
...
//  data->pipeline = gst_parse_launch("rtspsrc location=rtsp://192.168.168.230:8554/test latency=10 ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! autovideosink sync=false", &error);
  data->pipeline = gst_parse_launch("udpsrc multicast-group=224.0.0.252 port=8558 caps=\"application/x-rtp,media=video,clock-rate=90000,encoding-name=H264\" ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! autovideosink sync=false", &error);
...
app_function (void *userdata)
{
 ...
 /* Build pipeline */
 data->pipeline =
//  gst_parse_launch ("rtspsrc location=rtsp://192.168.168.230:8554/test latency=10 ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! autovideosink sync=false",
  // data->pipeline = gst_parse_launch("udpsrc multicast-group=224.0.0.252 port=8558 caps=\"application/x-rtp,media=video,clock-rate=90000,encoding-name=H264\" ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! autovideosink sync=false", &error);
    data->pipeline = gst_parse_launch(
            "rtspsrc location=rtsp://192.168.185.126:8554/test latency=10  ! "
...
gst_native_play (JNIEnv * env, jobject thiz)
{
 ...
 gst_element_set_state (data->pipeline, GST_STATE_PLAYING);
 __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "gst_native_play",data);
...
gst_native_pause (JNIEnv * env, jobject thiz)
{
 ...
 gst_element_set_state (data->pipeline, GST_STATE_PAUSED);
 __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "gst_native_pause",data);
...
static void gst_native_surface_init(JNIEnv *env, jobject thiz, jobject surface) {
 ...
 __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "gst_native_surface_init");

 // Store the surface object in CustomData
 if (data->surface_object) {
 (*env)->DeleteGlobalRef(env, data->surface_object);
 }
 data->surface_object = (*env)->NewGlobalRef(env, surface);
...
gst_native_surface_finalize (JNIEnv * env, jobject thiz)
{
 ...
 data->initialized = FALSE;
 __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "gst_native_surface_finalize");
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
 ...
 __android_log_print (ANDROID_LOG_ERROR, "GstPlayer", "Could not retrieve JNIEnv");
 return 0;
 }
 jclass klass = (*env)->FindClass (env, "com/kalyzee/gstreamer/GstPlayer");
